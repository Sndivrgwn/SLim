...
<aside class="sidenav navbar navbar-vertical navbar-expand-xs border-0 border-radius-xl my-3 fixed-start ms-3   bg-gradient-dark" id="sidenav-main">
    <div class="sidenav-header">
      ...
      <a class="navbar-brand m-0" href=" https://demos.creative-tim.com/material-dashboard/pages/dashboard " target="_blank">
        <img src="{{ asset('front/assets/img/logo-ct.png') }}" class="navbar-brand-img h-100" alt="main_logo">
        <span class="ms-1 font-weight-bold text-white">Forum</span>
      </a>
    </div>
    ...
    <div class="collapse navbar-collapse  w-auto " id="sidenav-collapse-main">
      <ul class="navbar-nav">
        ...
        <li class="nav-item">
          <a class="nav-link text-white " href="../pages/-in.html">
            <div class="text-white text-center me-2 d-flex align-items-center justify-content-center">
  ...
  <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg ">
      ...
      <div class="container-fluid py-1 px-3">
        ...
        <div class="collapse navbar-collapse mt-sm-0 mt-2 me-md-0 me-sm-4" id="navbar">
          ...
          <ul class="navbar-nav  justify-content-end">
              ...
              <ul class="dropdown-menu  dropdown-menu-end  px-2 py-3 me-sm-n4" aria-labelledby="dropdownMenuButton">
                <li class="mb-2">
                  <a class="dropdown-item border-radius-md" href="javascript:;">
                    <div class="d-flex py-1">
                      <div class="my-auto">
                        <img src="{{ asset('front/assets/img/team-2.jpg') }}" class="avatar avatar-sm  me-3 ">
                      </div>
                ...
                <li class="mb-2">
                  <a class="dropdown-item border-radius-md" href="javascript:;">
                    <div class="d-flex py-1">
                      <div class="my-auto">
                        <img src="{{ asset('front/assets/img/small-logos/logo-spotify.svg') }}" class="avatar avatar-sm bg-gradient-dark  me-3 ">
                      </div>
                      <div class="d-flex flex-column justify-content-center">
                        <h6 class="text-sm font-weight-normal mb-1">
            ...
            <li class="nav-item d-flex align-items-center">
              <a href="{{ route('users.dashboard') }}" class="nav-link text-body font-weight-bold px-0">
    ...
    <!-- End Navbar -->
    <h2 class="ms-5 text-uppercase mt-4 mb-3 text-dark fw-bold">SELAMAT DATANG, {{ Auth::User()->name }}!! </h2>
    <!-- USER TABLE -->
          ...
          <div class="card my-4">
            <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
              <div class="bg-gradient-primary shadow-primary border-radius-lg pt-4 pb-3 d-flex justify-content-between pe-3">
                <h6 class="text-white text-capitalize ps-3">thread</h6>
                @can('create posts', Thread::class)
                <a class="text-white fw-bold fs-6" href="{{ route('threads.create') }}"><i class="material-icons opacity-10 bg-success p-1 rounded rounded-2">add</i></a>
                @endcan
              </div>
            </div>
            <div class="card-body px-0 pb-2">
                    ...
                    <tr>
                      <td>
                        <div class="d-flex px-2 py-1">
                          ...
                          <div class="d-flex flex-column justify-content-center">
                            <h6 class="mb-0 text-sm">{{ Str::limit($thread->judul, 30, '...') }}</h6>
                          </div>
                        </div>
                      </td>
                      <td>
                        <p class="text-xs font-weight-bold mb-0">{{ Str::limit($thread->content, 50, '...') }}</p>
                      </td>
                      <td class="align-middle text-center text-sm">
                        <span class="badge badge-sm bg-gradient-dark">{{ $thread->creator->name }}</span>
                      </td>
                      <td class="align-middle text-center">
                        <span class="text-secondary text-xs font-weight-bold">{{ "$thread->created_at" }}</span>
                      </td>
                      <td class="align-middle d-flex align-items-center justify-content-center gap-2" colspan="2 ">
                          @can('edit posts', Thread::class)
                        <a href="{{ url('thread/edit/' . $thread->id) }}" class="text-light font-weight-bold text-xs badge bg-warning" data-toggle="tooltip" data-original-title="Edit user">
                        ...
                        <form action="{{ url('/thread/delete/' . $thread->id) }}" method="POST">
                          @csrf
                          @method('delete')
                            @can('delete posts', Thread::class)
                          <button type='submit' class=" font-weight-bold text-xs border-0 text-light badge bg-danger" data-toggle="tooltip">
                      ...
                      <td>
                        <a href="{{ url('thread/comment/add/' . $thread->id) }}"  class=" font-weight-bold text-xs border-0 text-light text-center d-flex justify-content-center align-items-center"><i class="material-icons opacity-10 badge bg-info p-2">comment</i></a>
                      </td>
                    </tr>
                  @endforeach
                  </tbody>
                  ...
                  @foreach($post as $pos)
                    <tr>
                      ...
                      <td class="align-middle d-flex align-items-center justify-content-center gap-2" colspan="2 ">
                        ...
                        <a href="{{ url('post/edit/' . $pos->id) }}" class="text-light font-weight-bold text-xs badge bg-warning" data-toggle="tooltip" data-original-title="Edit user">
                          <i class="material-icons opacity-10">create</i>
                        </a>
                        @endcan
                        @can('delete posts', Post::class)
                        <form action="{{ url('/post/delete/' . $pos->id) }}" method="POST">
                          @csrf
                          @method('delete')
                      ...
                      <td>
                        <a href="{{ url('post/comment/add/' . $pos->id) }}"  class=" font-weight-bold text-xs border-0 text-light text-center d-flex justify-content-center align-items-center"><i class="material-icons opacity-10 badge bg-info p-2">comment</i></a>
                      </td>
                    </tr>
                  @endforeach
                  </tbody>
                ...
                <table class="table align-items-center mb-0">
                  <thead>
                    <tr>
                      ...
                      <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">thread</th>
                  ...
                  @foreach($comments as $comment)
                    <tr>
                      ...
                      <td class="align-middle text-center text-sm">
                        @if ($comment->Post)
                            <span class="badge badge-sm bg-gradient-secondary">{{ Str::limit($comment->Post->judul ?? 'Judul tidak tersedia', 1000, '...') }}</span>
                        @else
                            <span class="badge badge-sm bg-gradient-danger" >Post tidak ditemukan</span>
                        @endif
                      </td>
                      <td class="align-middle text-center">
